...
func init(stock_level: int, size_mult: float = 1.0):
	...
	# 스폰 애니메이션: Tween 사용 (_process 대신 — 런타임 PackedScene에서 _process 미호출 이슈 우회)
	spawn_scale = 0.3
	gravity_scale = 0.0
	...
	tween.tween_property(self, "spawn_scale", 1.0, 0.12).set_ease(Tween.EASE_OUT)
...
func _on_body_entered(body: Node):
	# 버그 수정: 자신이 해제 중이면 무시
	if merging or not is_instance_valid(body):
		return
	if body is RigidBody2D and body.has_method("try_merge"):
		try_merge(body)

func try_merge(other):
	# 버그 수정: 양쪽 노드 유효성 + 부모 유효성 확인
	if not is_instance_valid(other) or not is_instance_valid(get_parent()):
		return
	if other.level == level and not merging and not other.merging:
		...
		if parent.has_method("merge_balls"):
			parent.merge_balls(self, other, merge_pos, level + 1)
